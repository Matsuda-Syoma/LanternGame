	...
	if (pflg == true && hit_soldier == false) {
		if (blinkingflg == false)
		{
			DrawRotaGraph((location.x * (1 - ((_distance / DISTANCE_MAX) / DISTANCE_NUM))) + (-loc.x + (SCREEN_WIDTH / 2))
						, (location.y * (1 - ((_distance / DISTANCE_MAX) / DISTANCE_NUM))) + (-loc.y + (SCREEN_HEIGHT / 2)), 1.0 * (1 - ((_distance / DISTANCE_MAX) / DISTANCE_NUM)), 0.0, playerimg[imgnum], true);
		}
		else {
		...
		DrawRotaGraph((location.x * (1 - ((_distance / DISTANCE_MAX) / DISTANCE_NUM))) + (-loc.x + (SCREEN_WIDTH / 2))
			, (location.y * (1 - ((_distance / DISTANCE_MAX) / DISTANCE_NUM))) + (-loc.y + (SCREEN_HEIGHT / 2)), 1.0 * (1 - ((_distance / DISTANCE_MAX) / DISTANCE_NUM)), 0.0, d_playerimg, true);
	}
	else if (pflg == true && hit_soldier == true)
	{
		DrawRotaGraph((location.x * (1 - ((_distance / DISTANCE_MAX) / DISTANCE_NUM))) + (-loc.x + (SCREEN_WIDTH / 2))
			, (location.y * (1 - ((_distance / DISTANCE_MAX) / DISTANCE_NUM))) + (-loc.y + (SCREEN_HEIGHT / 2)), 1.2 * (1 - ((_distance / DISTANCE_MAX) / DISTANCE_NUM)), 0.0, angryimg, true);
	}
